...
namespace weave
{
    ...
    public class ImageCache
    {
        ...
        HttpWebRequestQueue requestQueue = new HttpWebRequestQueue(4, CoreDispatcherScheduler.Current, Schedulers.ImageDownloaderScheduler);
                            ...
                            .SafelySubscribe(i =>
                            {
                                try
                                {
                                    ...
                                    var openedStream = Observable.FromEventPattern<RoutedEventArgs>(newBitmap, "ImageOpened")
                                        .Select(o => Tuple.Create<bool, string>(true, null));

                                    var closedStream = Observable.FromEventPattern<ExceptionRoutedEventArgs>(newBitmap, "ImageFailed")
                                        .Select(o => Tuple.Create<bool, string>(false, o.EventArgs.ErrorMessage));
                                    ...
                                    openedStream.Merge(closedStream).Take(1).SafelySubscribe(
                                        result =>
                                        {
                                            if (result.Item1)
                                            {
                                                ...
                                                observer.OnError(new Exception(result.Item2));
                                        },
                                        observer.OnError);
                                }
                                catch (Exception ex)
