...
namespace weave.Services.RSS
{
    public class FeedRequester
    {
        ...
        {
            return Observable.CreateWithDisposable<RequestStatus>(observer =>
            {
                var disposable = Disposable.Empty;
                try
                {
                    DebugEx.WriteLine(string.Format("Calling: {0}", this.FeedUri));
                    ...
                    disposable = FeedWebRequestServiceBus.Instance.Enqueue(request)
                        ...
                        .Subscribe(o =>
                        {
                            ...
                            {
                                DebugEx.WriteLine(string.Format("{0} call failed: {1}", this.FeedUri, ex.ToString()));
                                observer.OnError(ex);
                            }
                            finally
                            {
                ...
                {
                    DebugEx.WriteLine("Exception calling {0}: FeedSource.CheckForAndGetNewFeedData\r\n{1}",
                        FeedUri,
                        ex);
                    observer.OnError(ex);
                }
        ...
        {
            DebugEx.WriteLine(string.Format("### Using cached data for {0}", this.FeedUri));
            this.Status = RequestStatus.Unmodified;
        }

        void HandleNewData(HttpWebResponse o)
        {
            DebugEx.WriteLine(string.Format("Refreshing {0} with new data", this.FeedUri));
